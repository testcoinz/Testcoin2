...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a TestCoin2 address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no TestCoin2: URI
...
{
    // Convert testcoin2:// to testcoin2:
    //
    //    Cannot handle this later, because testcoin2:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open testcoin2.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TestCoin2.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for TestCoin2.lnk
...
{
    return GetAutostartDir() / "testcoin2.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a testcoin2.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=TestCoin2\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the testcoin2 app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add testcoin2 app to startup item list
