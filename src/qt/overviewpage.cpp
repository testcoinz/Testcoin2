...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeTestcoin2Amount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTestcoin2Amount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeTestcoin2Amount = strAnonymizeTestcoin2Amount.remove(strAnonymizeTestcoin2Amount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTestcoin2Amount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeTestcoin2Amount * COIN) nMaxToAnonymize = nAnonymizeTestcoin2Amount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeTestcoin2Amount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeTestcoin2Amount));
        strAnonymizeTestcoin2Amount = strAnonymizeTestcoin2Amount.remove(strAnonymizeTestcoin2Amount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTestcoin2Amount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeTestcoin2Amount == 0) {
